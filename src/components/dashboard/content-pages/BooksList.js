...
function BooksList() {
  ...
  let books = currentUser.extraData.books;
  ...
  const [currentPage,setCurrentPage] = useState(value_limit(parseInt(id, 10), minPage, maxPage)) 
  ...
  useEffect(() => {
    if (id !== undefined || currentPage !== 1) {
      history.push(`/mybooks/${currentPage || 1}`)
    }
    setBooksPerPage(getItems())
